...
<div x-data="{ 
...
}" @open-sidebar.window="mobileOpen = true" class="relative flex">
    ...
    <aside :class="{
        ...
        <!-- Sidebar header -->
        <div
            class="flex items-center justify-between h-16 px-4 bg-blue-600 dark:bg-blue-800 text-white shrink-0 overflow-hidden">
            <div class="flex items-center space-x-3 transition-all duration-300"
                :class="desktopCollapsed ? 'justify-center w-full' : ''">
                ...
                <span x-show="!desktopCollapsed" x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                    class="text-xl font-bold tracking-wider uppercase whitespace-nowrap">Apotek All</span>
            </div>
        ...
        <button @click="toggleDesktop()"
            class="hidden md:flex absolute -right-3 top-20 bg-white border border-gray-200 dark:bg-gray-700 dark:border-gray-600 rounded-full p-1 shadow-md z-50 hover:bg-gray-50 dark:hover:bg-gray-600 transition-transform duration-300"
            :style="desktopCollapsed ? 'transform: rotate(180deg)' : ''">
        ...
        <div class="flex-1 px-3 py-6 overflow-y-auto space-y-2 custom-scrollbar">
            ...
            <a href="{{ route('dashboard') }}" wire:navigate
                ...
                <svg class="w-6 h-6 shrink-0" :class="!desktopCollapsed && 'mr-3'" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
            ...
            <!-- Expired Products -->
            <a href="{{ route('admin.expired-products') }}" wire:navigate
                class="flex items-center justify-center md:justify-start px-4 py-3 rounded-lg transition-all duration-200 {{ request()->routeIs('admin.expired-products') ? 'bg-primary-600 text-white shadow-lg' : 'text-gray-300 hover:bg-white/10' }}">
                <svg class="w-6 h-6 shrink-0" :class="!desktopCollapsed && 'mr-3'" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span x-show="!desktopCollapsed" class="whitespace-nowrap">Monitor Expired</span>
            </a>
        ...
        <div class="p-4 border-t border-gray-100 dark:border-gray-700">
            <div :class="desktopCollapsed ? 'justify-center' : ''"
                class="flex items-center p-2 mb-4 bg-gray-50 dark:bg-gray-900 rounded-xl overflow-hidden">
                <div
                    class="h-10 w-10 shrink-0 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-blue-700 dark:text-blue-200 font-bold">
            ...
            <div :class="desktopCollapsed ? 'flex-col space-y-2' : 'flex space-x-2'" class="flex">
                <a href="{{ route('profile') }}" wire:navigate title="Profile"
                    class="flex-1 flex justify-center py-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                </a>
                <button wire:click="logout" title="Keluar"
                    class="flex-1 flex justify-center py-2 text-gray-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
